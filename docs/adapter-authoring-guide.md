...
- **`SessionSpec`** (engine → adapter) — `task_id`, `role` (`"dev"` / `"review"` /
  ...
  because no profile key maps it onto a CLI flag. `cursor-sdk` ignores it too: the SDK
  takes effort only inside the model id. `bmad-loop validate` warns when a stage on
  either family sets it. An out-of-tree adapter class may read it or not.
