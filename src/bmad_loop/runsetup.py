...
def make_adapters(project: Path, run_dir: Path, policy) -> dict[str, CodingCLIAdapter]:
    ...
    for role in ROLES:
        ...
        # A registered kind owns its transport (rather than declaring a tmux
        # profile), so route it before the generic profile/multiplexer path.
        kind = adapter_kinds.get_adapter_kind(cfg.name)
        if kind is not None:
            if kind.roles is not None and role not in kind.roles:
                supported = ", ".join(kind.roles)
                raise SystemExit(
