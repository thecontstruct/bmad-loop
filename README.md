...
[adapter]
name = "claude"            # CLI profile: claude | codex | gemini | copilot | antigravity | opencode-http (alias: opencode) | cursor-cli-headless | custom
...
| `cursor-cli-headless` | experimental — not yet E2E-verified against a live turn | **Cursor CLI** (`cursor-agent`, flags pinned against 2026.08.04), driven as a supervised child process in one-shot print mode (`-p --output-format stream-json`) — **no tmux window and no hooks**. Authenticate once with `cursor-agent login` or export `CURSOR_API_KEY`; skills live in `.cursor/skills/`. The terminal `result` frame is this transport's turn-end signal and carries the token counts (no transcript parser). Bypass flags (`--force --trust`) come from the profile, so `[adapter] extra_args` replaces them as usual. `-p` is one-shot, so a stalled session **cannot be nudged** — it runs out `session_timeout_min` instead. Watch sessions via `run_dir/logs/<task_id>.log` (the model's stream) and `<task_id>.err` (the CLI's own diagnostics). |
