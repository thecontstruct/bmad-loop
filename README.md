...
| `cursor`      | experimental                             | **Cursor CLI** (`cursor-agent`), verified against 2026.08.04. Skills live in `.cursor/skills/`; hooks in a project `.cursor/hooks.json`, whose top-level `version` is required — without it Cursor 3.x loads no hooks at all. `stop` is the turn-end event, `sessionStart` marks liveness. **`--trust` is mandatory for unattended runs**: an interactive launch in an untrusted directory blocks on a workspace-trust dialog and `--force` alone does _not_ clear it, so the profile ships `--force --trust` in its bypass flags — if you set `[adapter] extra_args` (which _replaces_ them) you must keep `--trust`. Because the flag is per-launch, `isolation = "worktree"` works, unlike antigravity. `usage_parser = "none"` for now: the Stop payload names a transcript, but its token schema is unread. Finalize with `probe-adapter cursor`. |
