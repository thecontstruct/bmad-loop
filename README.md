...
dev_stall_nudges = 2             # wake-nudges spent on grace expiry before a dev session is called stalled (a fresh Stop restores the budget)
dev_stall_nudges_cap = 6         # total never-restored stall nudges per dev/review session; bounds a reply-to-nudge that is itself a result-less Stop; 0 = stall on first grace expiry
...
[adapter]
name = "claude"            # CLI profile, or headless provider cursor-sdk; see provider table below
...
| `cursor-sdk`  | experimental                             | Cursor's local SDK agent, driven through a bundled Node sidecar — **no tmux window or hooks**. Requires Node ≥ 22.13, `CURSOR_API_KEY`, and `bmad-loop init --provision cursor-sdk` (which installs `@cursor/sdk` in `~/.bmad-loop/cursor-sdk` and seeds `.cursor/skills/`). The sidecar's terminal result is the completion signal; token usage comes from its SDK usage object. |
