...
[adapter]
name = "claude"            # CLI profile: claude | codex | gemini | copilot | antigravity | opencode-http (alias: opencode) | cursor-sdk | custom
...
effort = ""                # reasoning effort, free-form (e.g. "high", "max"); empty = provider default.
                           # Sent by opencode-http as the per-prompt `variant`; the tmux CLIs and
                           # cursor-sdk have no channel for it and ignore it (`bmad-loop validate`
                           # warns). For cursor-sdk, pick an effort-encoded model id instead.
...
| `BMAD_LOOP_CURSOR_SDK_DIR`    | directory path                                 | Overrides where the `cursor-sdk` adapter installs and loads its `@cursor/sdk` Node runtime — the directory `bmad-loop init --provision cursor-sdk` writes a `package.json` and `node_modules/` into, and the one the bundled sidecar is materialized beside. Used as the directory itself; `~` is expanded. Unset ⇒ `~/.bmad-loop/cursor-sdk`. Set it to share one provisioned runtime across machines-in-a-container, or to keep it off a home directory you do not control.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `cursor-sdk`  | experimental                             | **Cursor**'s headless agent, driven through a bundled Node sidecar over the [`@cursor/sdk`](https://www.npmjs.com/package/@cursor/sdk) package — **no tmux window, no hooks**. Needs Node ≥ 22.13, a `CURSOR_API_KEY`, and a one-off `bmad-loop init --provision cursor-sdk`, which installs the SDK under `~/.bmad-loop/cursor-sdk` (override with `BMAD_LOOP_CURSOR_SDK_DIR`). Skills read from `.claude/skills/`, which Cursor loads for compatibility. The sidecar's completion sentinel is the turn-end signal and carries the SDK's token usage. One run per session, so there are no wake-nudges: a session that ends without an artifact is a stall, not something to prod. Watch it via `run_dir/logs/<task_id>.log` or the TUI Log tab; `resolve` is `--no-interactive` only.                                                                                                    |
