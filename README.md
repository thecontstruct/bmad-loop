...
[adapter]
name = "claude"            # CLI profile, or the headless cursor-cli-headless provider
...
| `cursor-cli-headless` | experimental | Cursor CLI `cursor-agent -p --force --trust --output-format stream-json`, supervised directly with **no tmux window or hooks**. Install Cursor's headless CLI and authenticate with `cursor-agent login` or `CURSOR_API_KEY`; skills live in `.cursor/skills/`. The terminal `result` stream event supplies completion and token usage. |
